...
def generate_data(table_num, cdn_map):
    ...
    while True:
        ...
        print("start row: ", start_row, "end_row", end_row)
        client = bigquery.Client()
        try:
            query = (
                "SELECT pages.pageid as id, pages.url as siteURL, requests.url as requestURL, requests.mimeType as mimeType, requests.type as type, requests.resp_server as resp_server, requests.format as format,requests._cdn_provider as CDN FROM httparchive.summary_pages." + table_num + " pages INNER JOIN httparchive.summary_requests." + table_num + " requests ON pages.pageid = requests.pageid where pages.pageid > " + str(start_row) + " and pages.pageid < " + str(end_row)
            )
        ...
        i = start_row # get last index from the db
        try:
